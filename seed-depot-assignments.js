  ...
  try {
    ...
    for (let i = 0; i < drivers.length; i++) {
      ...
      driverToDepot[drivers[i].id] = {
        depotId: depotId,
        depotName: depots.find(d => d.id === depotId).depot_name,
    ...
    for (const route of routes) {
      ...
      for (let busIdx = 0; busIdx < busesPerRoute; busIdx++) {
        ...
        const [busResult] = await connection.query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [busNumber, busType, route.id, randomDepot.id, 50, assignedDriver, 'AVAILABLE']);
        ...
        for (const hour of departureHours) {
          ...
          await connection.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, 1)
          `, [route.id, busId, assignedDriver, randomDepot.id, depTime, arrTime]);
    ...
    const [allSchedules] = await connection.query(`
      SELECT s.id, s.bus_id, s.route_id, s.driver_id, s.departure_time, s.arrival_time, 
             s.depot_id, r.origin, r.destination, r.distance, r.estimated_duration, b.bus_type
      FROM schedules s
    ...
    const [firstDepotId] = await connection.query(`SELECT id FROM depots LIMIT 1`);
    
    if (firstDepotId.length > 0) {
      const depotId = firstDepotId[0].id;
      ...
      const [depotView] = await connection.query(`
        SELECT 
          ...
          s.departure_time, s.arrival_time, da.status,
      ...
      if (depotView.length > 0) {
        console.log(`📋 Sample Schedule (${depotView.length} trips):\n`);
    ...
    console.log('\n✅ Depot-based assignments seeded successfully!\n');
    ...
    console.log('   • Depot admins see only their location');
